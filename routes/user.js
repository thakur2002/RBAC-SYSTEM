...
// Get all users
router.get('/users', authenticate, authorizeRole(['getAllUsers']), async (req, res) => {
    try{
  ...
  router.put('/users/:username/:role', authenticate, authorizeRole(['updateRole']), async (req, res) => {
    ...
    try{
      const user = await User.findOneAndUpdate({username}, { role }, { new: true }).select('-password');
      if(!user){
        return res.status(404).json({error:"required user not found"});
      }
      res.status(200).json(user);
    }
    catch(e){
  ...
  // Delete a user
  router.delete('/users/:username', authenticate, authorizeRole(['deleteUser']), async (req, res) => {
